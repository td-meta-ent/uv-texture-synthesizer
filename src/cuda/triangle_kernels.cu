...
__global__ void ComputeTrianglePropertiesKernel(
  ...
  if (idx < *d_num_triangles * 3) {
    ...
    int d_vertex_indices[3] = {current_triangle[0], current_triangle[1],
                               current_triangle[2]};

    Eigen::Vector3d vertex_positions[3] = {d_vertices[d_vertex_indices[0]],
                                           d_vertices[d_vertex_indices[1]],
                                           d_vertices[d_vertex_indices[2]]};

    Eigen::Vector3d vertex_center, vertex_neighbor0, vertex_neighbor1;
    if (d_vertex_indices[0] == d_vertex_index) {
      vertex_center = vertex_positions[0];
      vertex_neighbor0 = vertex_positions[1];
      vertex_neighbor1 = vertex_positions[2];
    } else if (d_vertex_indices[1] == d_vertex_index) {
    ...
    double n0_opposite_side_length_squared = pow(
        GeometricUtils::compute_distance(vertex_center, vertex_neighbor1), 2);
    double n1_opposite_side_length_squared = pow(
        GeometricUtils::compute_distance(vertex_center, vertex_neighbor0), 2);
    ...
    // Collinear Vertices
    if (abs(d_triangle_properties[idx].normal[0]) == 0.0 &&
        abs(d_triangle_properties[idx].normal[1]) == 1.0 &&
        abs(d_triangle_properties[idx].normal[2]) == 0.0) {
      if (isnan(d_triangle_properties[idx].curvature_normal_tmp[0])) {
        d_triangle_properties[idx].curvature_normal_tmp = Eigen::Vector3d(
            0.00000000000000001, 0.00000000000000001, 0.00000000000000001);
        d_triangle_properties[idx].mixed_area = 0.000000001;
      }
    }
  }
}
...
__global__ void StoreFirstNormalTrianglePropertiesKernel(
    const int *d_num_triangles, TriangleProperties *d_triangle_properties,
    TriangleProperties *d_triangle_properties_fixed_normal) {
  int idx = static_cast<int>(blockIdx.x) * static_cast<int>(blockDim.x) +
            static_cast<int>(threadIdx.x);

  if (idx >= *d_num_triangles * 3) return;

  d_triangle_properties_fixed_normal[idx].normal =
      d_triangle_properties[idx].normal;
}

__global__ void AssignFirstNormalTrianglePropertiesKernel(
    const int *d_num_triangles, TriangleProperties *d_triangle_properties,
    TriangleProperties *d_triangle_properties_fixed_normal) {
  int idx = static_cast<int>(blockIdx.x) * static_cast<int>(blockDim.x) +
            static_cast<int>(threadIdx.x);
...
void LaunchAssignFirstNormalTrianglePropertiesKernel(
    ...
    int block_size) {
  AssignFirstNormalTrianglePropertiesKernel<<<grid_size, block_size>>>(
      d_num_triangles, d_triangle_properties,
      d_triangle_properties_fixed_normal);
...
void LaunchStoreFirstNormalTrianglePropertiesKernel(
    ...
    TriangleProperties *d_triangle_properties_fixed_normal, int grid_size,
    int block_size) {
  StoreFirstNormalTrianglePropertiesKernel<<<grid_size, block_size>>>(
      d_num_triangles, d_triangle_properties,
      d_triangle_properties_fixed_normal);
